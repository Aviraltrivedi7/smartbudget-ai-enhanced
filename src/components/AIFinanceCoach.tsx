...
const AIFinanceCoach: React.FC<AIFinanceCoachProps> = ({ onBack, transactions, initialPrompt = '' }) => {
  ...
  useEffect(() => {
    setMessages([{ id: 'welcome', type: 'ai', content: language === 'hi' ? 'Namaste! Main aapka DhanSetu AI Coach hoon. Aap apne expenses, spending patterns, budget ya savings goals ke baare mein seedha pooch sakte hain.' : 'Hey, I’m your DhanSetu AI Coach. Ask me anything about your expenses, spending patterns, budget, or savings goals.', timestamp: new Date(), suggestions }]);
  ...
  return <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-10 lg:py-9">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><button onClick={onBack} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-slate-500 transition hover:text-[#5867bb]"><ArrowLeft className="h-4 w-4" /> Back to overview</button><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#222d4b] text-[#dfe4ff] shadow-lg shadow-teal-950/10"><Bot className="h-5 w-5" /></span><div><p className="eyebrow">DhanSetu AI Coach</p><h1 className="mt-1 text-4xl font-semibold tracking-[-0.05em] text-slate-950 sm:text-5xl">Talk to your money.</h1></div></div><p className="mt-4 max-w-2xl text-sm leading-6 text-slate-500">Ask clear questions, get answers grounded in your tracked transactions, and turn the next best move into an action.</p></div><div className="flex items-center gap-2"><button onClick={toggleLanguage} className="inline-flex items-center gap-2 rounded-xl border border-[#e7e8ee] bg-white px-3.5 py-2.5 text-xs font-bold text-slate-600 transition hover:border-[#c3cbee] hover:text-[#5867bb]"><Languages className="h-4 w-4" /> {language === 'hi' ? 'English' : 'हिंदी / Hinglish'}</button><span className={cn('inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-bold', providerStatus === 'external' ? 'border border-[#d2d6ef] bg-[#e9eefb] text-[#5867bb]' : 'border border-orange-100 bg-[#f7efe7] text-[#a65c4e]')}><span className={cn('h-2 w-2 rounded-full', providerStatus === 'external' ? 'animate-pulse bg-[#5867bb]' : 'bg-[#bf7864]')} /> {providerStatus === 'external' ? 'External AI connected' : 'Local fallback ready'}</span></div></div>
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.65fr)]"><section className="premium-card flex min-h-[620px] flex-col overflow-hidden"><div className="flex items-center justify-between border-b border-[#ececf1] px-5 py-4 sm:px-7"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#e9eefb] text-[#5867bb]"><MessageCircle className="h-4 w-4" /></div><div><p className="text-sm font-bold text-slate-900">Live expense chat</p><p className="text-xs text-slate-400">Answers from your current money context</p></div></div><span className="hidden items-center gap-1.5 text-xs font-semibold text-slate-400 sm:flex"><ShieldCheck className="h-4 w-4 text-[#5867bb]" /> Private workspace</span></div><div className="flex-1 space-y-5 overflow-y-auto px-5 py-6 sm:px-7">{messages.map((message) => <div key={message.id} className={cn('flex gap-3', message.type === 'user' && 'justify-end')}><div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-xl', message.type === 'ai' ? 'bg-[#222d4b] text-[#dfe4ff]' : 'bg-[#eef0eb] text-slate-500 order-2')} >{message.type === 'ai' ? <Bot className="h-4 w-4" /> : <UserRound className="h-4 w-4" />}</div><div className={cn('max-w-[86%] rounded-2xl px-4 py-3 text-sm leading-6', message.type === 'ai' ? 'rounded-tl-sm bg-[#f8f7f4] text-slate-700' : 'rounded-tr-sm bg-[#222d4b] text-white')}><p className="whitespace-pre-line">{message.content}</p><p className={cn('mt-2 text-[10px] font-semibold', message.type === 'ai' ? 'text-slate-400' : 'text-white/65')}>{message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div></div>)}{isTyping && <div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#222d4b] text-[#dfe4ff]"><Bot className="h-4 w-4" /></div><div className="flex gap-1 rounded-2xl rounded-tl-sm bg-[#f8f7f4] px-4 py-4"><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#5867bb]" /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#5867bb] [animation-delay:120ms]" /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#5867bb] [animation-delay:240ms]" /></div></div>}<div ref={messagesEndRef} /></div><div className="border-t border-[#ececf1] bg-[#f8f7f4]/60 px-5 py-3 sm:px-7">{!isTyping && <div className="flex gap-2 overflow-x-auto pb-3">{(messages[messages.length - 1]?.suggestions || suggestions).slice(0, 3).map((suggestion) => <button key={suggestion} onClick={() => sendMessage(suggestion)} className="shrink-0 rounded-full border border-[#e7e8ee] bg-white px-3 py-2 text-xs font-semibold text-slate-600 transition hover:border-[#aeb8ed] hover:text-[#5867bb]">{suggestion}</button>)}</div>}<div className="flex items-center gap-2 rounded-2xl border border-[#e7e8ee] bg-white p-1.5 shadow-sm"><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && sendMessage()} placeholder={language === 'hi' ? 'Apne expenses ke baare mein poochiye...' : 'Ask about your expenses...'} className="min-w-0 flex-1 bg-transparent px-3 py-2 text-sm text-slate-800 outline-none placeholder:text-slate-400" /><button onClick={() => sendMessage()} disabled={!input.trim() || isTyping} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#222d4b] text-white transition hover:bg-[#3e4c91] disabled:cursor-not-allowed disabled:opacity-40"><Send className="h-4 w-4" /></button></div></div></section><aside className="space-y-5"><div className="rounded-[1.5rem] bg-[#222d4b] p-6 text-white shadow-[0_18px_45px_rgba(34,45,75,0.18)]"><div className="flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#dfe4ff]/60">Live context</p><Sparkles className="h-5 w-5 text-[#dfe4ff]" /></div><p className="mt-6 text-3xl font-semibold tracking-tight">{money(Math.max(0, summary.income - summary.expenses))}</p><p className="mt-1 text-sm text-white/70">available balance</p><div className="mt-7 grid grid-cols-2 gap-3 border-t border-white/10 pt-5"><div><p className="text-[10px] uppercase tracking-wider text-white/60">Income</p><p className="mt-1 font-bold">{money(summary.income)}</p></div><div><p className="text-[10px] uppercase tracking-wider text-white/60">Expenses</p><p className="mt-1 font-bold">{money(summary.expenses)}</p></div></div></div><div className="premium-card p-5"><div className="flex items-center justify-between"><div><p className="eyebrow">DhanSetu knows</p><h2 className="mt-2 text-lg font-semibold text-slate-950">Your money snapshot</h2></div><TrendingUp className="h-5 w-5 text-[#5867bb]" /></div><div className="mt-5 space-y-4"><Snapshot icon={Wallet} label="Transactions tracked" value={transactions.length.toString()} /><Snapshot icon={TrendingUp} label="Top category" value={summary.topCategory?.[0] || 'Add more data'} detail={summary.topCategory ? money(summary.topCategory[1]) : undefined} /><Snapshot icon={ShieldCheck} label="Context status" value="Ready to answer" /></div><button onClick={() => sendMessage(language === 'hi' ? 'Mera balance kitna hai?' : 'What is my current balance?')} className="mt-5 flex w-full items-center justify-between rounded-xl bg-[#f8f7f4] px-3.5 py-3 text-left text-xs font-bold text-slate-600 transition hover:bg-[#e9eefb] hover:text-[#5867bb]">Ask a quick question <ChevronRight className="h-4 w-4" /></button></div></aside></div>
  </div>;
};

const Snapshot = ({ icon: Icon, label, value, detail }: { icon: React.ElementType; label: string; value: string; detail?: string }) => <div className="flex items-center gap-3"><div className="rounded-xl bg-[#e9eefb] p-2 text-[#5867bb]"><Icon className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="text-xs text-slate-400">{label}</p><p className="truncate text-sm font-bold text-slate-800">{value}{detail && <span className="ml-1 font-semibold text-slate-400">· {detail}</span>}</p></div></div>;
