...
const VoiceTransactionEntry: React.FC<VoiceTransactionEntryProps> = ({ onBack, onSave }) => {
  ...
  const startListening = () => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.onstart = () => {
        setIsListening(true);
        toast({
          title: "Sunaayi de raha hai... 🎤",
          description: "Abhi bolo jo record karna hai.",
        });
      };
  ...
  const parseVoiceInput = (text: string) => {
    ...
    setTimeout(() => {
      ...
      toast({
        title: "Voice Pakad Li! 🎯",
        description: `Maine suna: ${category} kharcha of ₹${amount}`,
      });
    }, 1500);
  };
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 font-medium">Bolo aur AI sab record kar lega!</p>
          </div>
        </div>
            ...
            <CardContent className="space-y-6">
              <div className="text-center">
                <Button
                  ...
                  className={`w-32 h-32 rounded-full text-white text-lg font-medium transition-all duration-300 ${isListening
