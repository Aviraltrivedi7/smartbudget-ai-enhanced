...
const Dashboard: React.FC<DashboardProps> = memo(({ transactions: propTransactions, onNavigate, onShowWelcomeGuide, onOpenBudgetPlanner, onTransactionSelect }) => {
  ...
  const savingsRate = totals.income ? Math.max(0, Math.round(((totals.income - totals.expenses) / totals.income) * 100)) : 0;
  ...
  const quietCategory = ['Travel', 'Food', 'Rent', 'Utilities'].find((category) => !categoryData.some((entry) => entry.rawName.toLowerCase() === category.toLowerCase()));
  const financialHealth = filteredTransactions.length ? Math.min(100, Math.round((savingsRate * 0.6) + (Math.min(loggingDays, 7) * 4) + (leadingCategory ? 10 : 0))) : 0;
  ...
  const smartSuggestions = [
    ...
    { label: 'Weekly pulse', title: expenseRatio > 100 ? 'Your spend is moving faster than income.' : 'Your cash flow has room to breathe.', detail: expenseRatio > 100 ? 'Try one low-spend day this week and watch the trend line respond.' : 'Keep this rhythm and review your biggest category every Sunday.', tone: 'orange' },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-10 lg:py-9">
      <section className="dashboard-reveal dashboard-reveal-1 hero-panel relative overflow-hidden rounded-[1.5rem] px-6 py-7 text-white shadow-[0_24px_70px_rgba(15,23,42,0.2)] sm:px-9 sm:py-9">
        ...
        <div className="relative z-10 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
            ...
            <button onClick={() => onNavigate?.('monthly-report')} className="inline-flex items-center gap-2 rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/15"><FileDown className="h-4 w-4" /> Monthly report</button>
      ...
      <section className="dashboard-reveal dashboard-reveal-2 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Net balance" value={currency(totals.balance)} note={totals.balance >= 0 ? 'Available after expenses' : 'Expenses exceed income'} icon={Wallet} tone="dark" trend={totals.balance >= 0 ? 'On track' : 'Needs review'} />
        ...
        <MetricCard label="Total expenses" value={currency(totals.expenses)} note={totals.income ? `${expenseRatio}% of income used` : 'Add income to compare'} icon={ArrowUpRight} tone="sand" trend={expenseRatio ? `${expenseRatio}%` : '0%'} />
      ...
      <section className="dashboard-reveal dashboard-reveal-3 grid gap-5 lg:grid-cols-2">
        ...
        <div className="premium-card p-6 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Goal tracker</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Build your buffer.</h2><p className="mt-1 text-sm text-slate-500">A small ₹500 cushion makes the next surprise easier.</p></div><Target className="h-5 w-5 text-[#5867bb]" /></div><div className="mt-5 flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">Buffer goal</p><p className="mt-1 text-2xl font-semibold text-slate-950">{currency(bufferSaved)} <span className="text-sm font-medium text-slate-400">/ ₹500</span></p></div><span className="text-sm font-bold text-[#5867bb]">{Math.round((bufferSaved / 500) * 100)}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e7e8ee]"><div className="h-full rounded-full bg-[#5867bb] transition-all duration-300" style={{ width: `${Math.min(100, (bufferSaved / 500) * 100)}%` }} /></div><button type="button" onClick={addBufferAmount} disabled={bufferSaved >= 500} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#222d4b] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-[#3e4c91] disabled:cursor-not-allowed disabled:opacity-50"><Plus className="h-4 w-4" />{bufferSaved >= 500 ? 'Goal complete' : 'Add ₹100 now'}</button></div>
      </section>
