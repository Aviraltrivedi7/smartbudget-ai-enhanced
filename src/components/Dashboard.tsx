...
const chartColors = ['#397568', '#7b9b88', '#b56a4e', '#a18d78', '#6f8580', '#c4b9a3'];
        ...
        <div className="relative z-10 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-5 flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-[0.22em] text-teal-100/80">
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5"><span className="h-1.5 w-1.5 rounded-full bg-[#9acdb6] shadow-[0_0_14px_#9acdb6]" />{statusLabel}</span>
          ...
          <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
            ...
            <button onClick={() => onNavigate?.('add-expense')} className="inline-flex items-center gap-2 rounded-2xl bg-[#cfe8dc] px-5 py-3 text-sm font-bold text-[#193b35] shadow-lg shadow-teal-950/20 transition hover:-translate-y-0.5 hover:bg-white active:scale-[0.98]"><Plus className="h-4 w-4" /> Add transaction</button>
          </div>
        </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.9fr)]">
        <div className="premium-card min-h-[410px] p-5 sm:p-7">
          ...
          <div className="h-[285px] w-full">
            {chartView === 'trend' ? (
              trendData.length ? <ResponsiveContainer width="100%" height="100%"><AreaChart data={trendData} margin={{ top: 10, right: 6, left: -22, bottom: 0 }}><defs><linearGradient id="incomeFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#397568" stopOpacity={0.28} /><stop offset="100%" stopColor="#397568" stopOpacity={0} /></linearGradient><linearGradient id="expenseFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#b56a4e" stopOpacity={0.2} /><stop offset="100%" stopColor="#b56a4e" stopOpacity={0} /></linearGradient></defs><CartesianGrid vertical={false} stroke="#e3e7e1" strokeDasharray="3 3" /><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: '#8b9690', fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: '#8b9690', fontSize: 11 }} tickFormatter={(value) => `₹${value >= 1000 ? `${value / 1000}k` : value}`} /><Tooltip contentStyle={{ borderRadius: 16, border: '1px solid #e7e6df', boxShadow: '0 12px 30px rgba(15,23,42,.12)' }} formatter={(value: number, name: string) => [currency(value), name === 'income' ? 'Income' : 'Expenses']} /><Area type="monotone" dataKey="income" stroke="#397568" strokeWidth={3} fill="url(#incomeFill)" /><Area type="monotone" dataKey="expenses" stroke="#b56a4e" strokeWidth={2.5} fill="url(#expenseFill)" /></AreaChart></ResponsiveContainer> : <EmptyChart label="Add a few transactions to see your cash flow." />
            ) : (
              categoryData.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={categoryData} margin={{ top: 10, right: 6, left: -22, bottom: 0 }}><CartesianGrid vertical={false} stroke="#e3e7e1" strokeDasharray="3 3" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#697972', fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: '#8b9690', fontSize: 11 }} tickFormatter={(value) => `₹${value >= 1000 ? `${value / 1000}k` : value}`} /><Tooltip cursor={{ fill: '#f5f4ef' }} contentStyle={{ borderRadius: 16, border: '1px solid #e7e6df' }} formatter={(value: number) => [currency(value), 'Spent']} /><Bar dataKey="value" radius={[8, 8, 3, 3]}>{categoryData.map((entry) => <Cell key={entry.name} fill={entry.fill} />)}</Bar></BarChart></ResponsiveContainer> : <EmptyChart label="Categories will appear once you log spending." />
            )}
          </div>
          <div className="mt-4 flex flex-wrap gap-4 text-xs font-semibold text-slate-500"><span className="inline-flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#397568]" />Income</span><span className="inline-flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#b56a4e]" />Expenses</span><span className="ml-auto text-slate-400">Tap chart to explore</span></div>
        </div>

        <div className="premium-card p-5 sm:p-7">
          <div className="flex items-start justify-between"><div><p className="eyebrow">Spending pulse</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Where it goes</h2></div><div className="rounded-xl bg-[#e8f2ed] p-2.5 text-[#397568]"><BarChart3 className="h-5 w-5" /></div></div>
          <div className="relative mx-auto mt-5 h-52 w-52">{categoryData.length ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={categoryData} dataKey="value" innerRadius={64} outerRadius={88} paddingAngle={4} stroke="none">{categoryData.map((entry) => <Cell key={entry.name} fill={entry.fill} />)}</Pie></PieChart></ResponsiveContainer> : <div className="flex h-full items-center justify-center rounded-full border-[18px] border-[#eeeee8]"><div className="text-center"><p className="text-2xl font-semibold text-slate-900">0%</p><p className="text-xs text-slate-400">tracked</p></div></div>}<div className="pointer-events-none absolute inset-0 flex items-center justify-center"><div className="text-center"><p className="text-2xl font-semibold tracking-tight text-slate-950">{currency(totals.expenses)}</p><p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">spent</p></div></div></div>
          <div className="mt-4 space-y-3">{categoryData.length ? categoryData.slice(0, 4).map((category) => <div key={category.name} className="flex items-center justify-between text-sm"><span className="flex min-w-0 items-center gap-2.5 text-slate-600"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: category.fill }} />{category.name}</span><span className="font-semibold text-slate-900">{currency(category.value)}</span></div>) : <p className="rounded-2xl bg-[#f7f6f2] px-4 py-3 text-sm leading-5 text-slate-500">Your spending breakdown will become more useful as you add transactions.</p>}</div>
        </div>
      </section>

      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="premium-card p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">AI copilot</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Your next best moves</h2><p className="mt-1 text-sm text-slate-500">Personalized nudges from your current money pattern.</p></div><button onClick={() => onNavigate?.('coach')} className="inline-flex shrink-0 items-center gap-1.5 rounded-xl bg-[#193b35] px-3 py-2 text-xs font-bold text-white transition hover:bg-teal-800"><MessageCircle className="h-3.5 w-3.5" /> Live chat</button></div><div className="mt-6 space-y-3">{smartSuggestions.map((suggestion, index) => <button key={suggestion.label} onClick={() => onNavigate?.(index === 1 ? 'savings-goals' : index === 0 ? 'budget-planner' : 'insights')} className="group flex w-full items-center gap-3 rounded-2xl border border-[#eeeee8] bg-[#f7f6f2]/60 p-3.5 text-left transition hover:-translate-y-0.5 hover:border-[#e7e6df] hover:bg-white hover:shadow-[0_10px_24px_rgba(15,23,42,0.06)]"><span className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-xl', suggestion.tone === 'teal' ? 'bg-[#e8f2ed] text-[#397568]' : suggestion.tone === 'violet' ? 'bg-[#eff3ee] text-[#5b7d6e]' : 'bg-[#f5eee8] text-[#a65e43]')}><Sparkles className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">{suggestion.label}</span><span className="mt-1 block text-sm font-bold text-slate-800">{suggestion.title}</span><span className="mt-1 block text-xs leading-5 text-slate-400">{suggestion.detail}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-[#397568]" /></button>)}</div></div>
        <div className="premium-card p-5 sm:p-7"><div className="flex items-start justify-between"><div><p className="eyebrow">Transaction rhythm</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Your week at a glance</h2></div><div className="rounded-xl bg-[#f5eee8] p-2.5 text-[#a65e43]"><Waves className="h-5 w-5" /></div></div><div className="mt-5 rounded-2xl bg-[#f7f6f2]/80 px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Selected day</p><p className="mt-1 text-lg font-semibold text-slate-900">{activeRhythm?.day ? `Day ${activeRhythm.day}` : 'No activity yet'} <span className="text-sm font-medium text-slate-400">· {currency(activeRhythm?.amount || 0)} spent</span></p></div><div className="mt-7 flex h-40 items-end gap-2">{weeklyData.map((point) => { const isSelected = selectedRhythmDay === point.day || (selectedRhythmDay === null && point.day === activeRhythm?.day); const height = point.amount ? Math.max(16, (point.amount / maxWeeklySpend) * 100) : 8; return <button key={point.day} onClick={() => setSelectedRhythmDay(point.day)} className="group flex h-full flex-1 flex-col items-center justify-end gap-2"><span className={cn('relative block w-full max-w-8 rounded-t-lg transition-all duration-300', isSelected ? 'bg-[#193b35] shadow-[0_8px_16px_rgba(16,43,41,0.18)]' : 'bg-[#b7d3c4] group-hover:bg-[#6f9c88]')} style={{ height: `${height}%` }}>{isSelected && point.amount > 0 && <span className="absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-bold text-slate-500">{currency(point.amount)}</span>}</span><span className={cn('text-[10px] font-bold', isSelected ? 'text-slate-800' : 'text-slate-400')}>{point.day}</span></button>; })}</div><div className="mt-5 flex items-center justify-between text-xs text-slate-400"><span className="inline-flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#193b35]" /> Selected</span><span>Click a bar to inspect</span></div></div>
      </section>

      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        <div className="premium-card p-5 sm:p-7"><div className="mb-5 flex items-center justify-between"><div><p className="eyebrow">Activity</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Recent transactions</h2></div><button onClick={() => onNavigate?.('calendar-tracker')} className="inline-flex items-center gap-1 text-sm font-bold text-[#397568] transition hover:text-[#193b35]">View all <ChevronRight className="h-4 w-4" /></button></div>{recentTransactions.length ? <div className="divide-y divide-slate-100">{recentTransactions.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} currentLanguage={currentLanguage} />)}</div> : <EmptyActivity onAdd={() => onNavigate?.('add-expense')} />}</div>
        <div className="relative overflow-hidden rounded-[1.5rem] bg-[#193b35] p-6 text-white shadow-[0_18px_45px_rgba(16,43,41,0.18)] sm:p-7"><div className="absolute -right-14 -top-14 h-40 w-40 rounded-full border-[22px] border-[#9acdb6]/10" /><div className="absolute -bottom-20 -left-10 h-44 w-44 rounded-full border-[28px] border-orange-200/10" /><div className="relative z-10"><div className="mb-10 flex items-center justify-between"><div className="rounded-xl bg-white/10 p-2.5"><Sparkles className="h-5 w-5 text-teal-200" /></div><span className="rounded-full border border-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-teal-100/70">AI insight</span></div><p className="text-sm font-medium text-teal-100/65">One small shift, big impact</p><h3 className="mt-3 text-2xl font-semibold leading-tight tracking-tight">Your next best move is consistency, not restriction.</h3><p className="mt-4 text-sm leading-6 text-white/55">Keep logging for seven days and SmartBudget will surface a clearer pattern in your spending.</p><button onClick={() => onNavigate?.('insights')} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-bold text-[#193b35] transition hover:-translate-y-0.5">Explore insights <ChevronRight className="h-4 w-4" /></button></div></div>
      </section>
...
const TransactionRow = ({ transaction, currentLanguage }: { transaction: Transaction; currentLanguage: string }) => <div className="flex items-center justify-between gap-3 py-3.5 first:pt-0 last:pb-0"><div className="flex min-w-0 items-center gap-3"><div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-xl', transaction.type === 'income' ? 'bg-[#e8f2ed] text-[#397568]' : 'bg-[#f5eee8] text-[#a65e43]')}>{transaction.type === 'income' ? <ArrowDownRight className="h-4 w-4" /> : <CreditCard className="h-4 w-4" />}</div><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-800">{transaction.title}</p><p className="mt-0.5 truncate text-xs text-slate-400">{getCategoryTranslation(transaction.category, currentLanguage)} · {new Date(transaction.date).toLocaleDateString()}</p></div></div><p className={cn('shrink-0 text-sm font-bold', transaction.type === 'income' ? 'text-[#397568]' : 'text-slate-900')}>{transaction.type === 'income' ? '+' : '-'}{currency(transaction.amount)}</p></div>;

const Shortcut = ({ icon: Icon, title, description, onClick, tone }: { icon: React.ElementType; title: string; description: string; onClick: () => void; tone: 'teal' | 'violet' | 'orange' | 'blue' }) => <button onClick={onClick} className="group flex items-center gap-4 rounded-2xl border border-[#e7e6df]/80 bg-white p-4 text-left shadow-[0_5px_20px_rgba(15,23,42,0.035)] transition duration-200 hover:-translate-y-0.5 hover:border-[#d2d9d3] hover:shadow-[0_14px_30px_rgba(15,23,42,0.08)]"><div className={cn('rounded-xl p-3', tone === 'teal' && 'bg-[#e8f2ed] text-[#397568]', tone === 'violet' && 'bg-[#eff3ee] text-[#5b7d6e]', tone === 'orange' && 'bg-[#f5eee8] text-[#a65e43]', tone === 'blue' && 'bg-[#eef2f1] text-[#54736a]')}><Icon className="h-5 w-5" /></div><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-slate-800">{title}</span><span className="mt-0.5 block text-xs text-slate-400">{description}</span></span><ChevronRight className="h-4 w-4 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-slate-500" /></button>;

const EmptyChart = ({ label }: { label: string }) => <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-[#e7e6df] bg-[#f7f6f2]/60 px-6 text-center text-sm text-slate-400"><Minus className="mr-2 h-4 w-4" />{label}</div>;
const EmptyActivity = ({ onAdd }: { onAdd: () => void }) => <div className="flex min-h-[190px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#e7e6df] bg-[#f7f6f2]/60 px-6 text-center"><div className="mb-3 rounded-xl bg-white p-3 text-slate-400 shadow-sm"><IndianRupee className="h-5 w-5" /></div><p className="text-sm font-semibold text-slate-700">Your money story starts here.</p><p className="mt-1 max-w-xs text-xs leading-5 text-slate-400">Add your first transaction and we’ll turn it into a useful signal.</p><button onClick={onAdd} className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-[#397568]">Add transaction <Plus className="h-3.5 w-3.5" /></button></div>;
