  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7 px-4 py-6 sm:px-6 lg:px-10 lg:py-9">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="premium-card p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">AI copilot</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Your next best moves</h2><p className="mt-1 text-sm text-slate-500">Personalized nudges from your current money pattern.</p></div><button onClick={() => onNavigate?.('coach')} className="inline-flex shrink-0 items-center gap-1.5 rounded-xl bg-[#102b29] px-3 py-2 text-xs font-bold text-white transition hover:bg-teal-800"><MessageCircle className="h-3.5 w-3.5" /> Live chat</button></div><div className="mt-6 space-y-3">{smartSuggestions.map((suggestion, index) => <button key={suggestion.label} onClick={() => onNavigate?.(index === 1 ? 'savings-goals' : index === 0 ? 'budget-planner' : 'insights')} className="group flex w-full items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/60 p-3.5 text-left transition hover:-translate-y-0.5 hover:border-slate-200 hover:bg-white hover:shadow-[0_10px_24px_rgba(15,23,42,0.06)]"><span className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-xl', suggestion.tone === 'teal' ? 'bg-teal-50 text-teal-700' : suggestion.tone === 'violet' ? 'bg-violet-50 text-violet-700' : 'bg-orange-50 text-orange-600')}><Sparkles className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">{suggestion.label}</span><span className="mt-1 block text-sm font-bold text-slate-800">{suggestion.title}</span><span className="mt-1 block text-xs leading-5 text-slate-400">{suggestion.detail}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-teal-600" /></button>)}</div></div>
        <div className="premium-card p-5 sm:p-7"><div className="flex items-start justify-between"><div><p className="eyebrow">Transaction rhythm</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Your week at a glance</h2></div><div className="rounded-xl bg-orange-50 p-2.5 text-orange-600"><Waves className="h-5 w-5" /></div></div><div className="mt-5 rounded-2xl bg-slate-50/80 px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Selected day</p><p className="mt-1 text-lg font-semibold text-slate-900">{activeRhythm?.day ? `Day ${activeRhythm.day}` : 'No activity yet'} <span className="text-sm font-medium text-slate-400">· {currency(activeRhythm?.amount || 0)} spent</span></p></div><div className="mt-7 flex h-40 items-end gap-2">{weeklyData.map((point) => { const isSelected = selectedRhythmDay === point.day || (selectedRhythmDay === null && point.day === activeRhythm?.day); const height = point.amount ? Math.max(16, (point.amount / maxWeeklySpend) * 100) : 8; return <button key={point.day} onClick={() => setSelectedRhythmDay(point.day)} className="group flex h-full flex-1 flex-col items-center justify-end gap-2"><span className={cn('relative block w-full max-w-8 rounded-t-lg transition-all duration-300', isSelected ? 'bg-[#102b29] shadow-[0_8px_16px_rgba(16,43,41,0.18)]' : 'bg-teal-200 group-hover:bg-teal-400')} style={{ height: `${height}%` }}>{isSelected && point.amount > 0 && <span className="absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-bold text-slate-500">{currency(point.amount)}</span>}</span><span className={cn('text-[10px] font-bold', isSelected ? 'text-slate-800' : 'text-slate-400')}>{point.day}</span></button>; })}</div><div className="mt-5 flex items-center justify-between text-xs text-slate-400"><span className="inline-flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#102b29]" /> Selected</span><span>Click a bar to inspect</span></div></div>
      </section>
