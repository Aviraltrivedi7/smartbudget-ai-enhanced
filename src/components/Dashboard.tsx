...
const Dashboard: React.FC<DashboardProps> = memo(({ transactions: propTransactions, onNavigate, onShowWelcomeGuide, onOpenBudgetPlanner, onOpenTransactionModal, onTransactionSelect }) => {
  ...
  const monthLabel = `${selectedMonthName.charAt(0).toUpperCase()}${selectedMonthName.slice(1)} ${selectedYearValue}`;
  const hasMoneyData = Boolean(totals.income || totals.expenses);
  const heroHeadline = hasMoneyData ? 'Here’s your month, in plain view.' : 'Start with one small money habit.';
  const heroDescription = hasMoneyData ? `You brought in ${currency(totals.income)} and spent ${currency(totals.expenses)} in ${monthLabel}. Here are the few categories worth your attention.` : 'Add an income or expense and DhanSetu will turn it into a clear, useful picture.';
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-10 lg:py-9">
      <section className="dashboard-reveal dashboard-reveal-1 hero-panel relative overflow-hidden rounded-[1.5rem] px-6 py-7 text-white shadow-[0_24px_70px_rgba(15,23,42,0.2)] sm:px-9 sm:py-9">
        ...
        <div className="relative z-10 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mb-2 text-sm font-medium text-white/75">{hasMoneyData ? `A quick read of ${monthLabel}` : 'A calmer place to start'}</p>
            <h1 className="max-w-xl text-4xl font-semibold tracking-[-0.055em] sm:text-5xl">{heroHeadline}</h1>
            <p className="mt-4 max-w-lg text-sm leading-6 text-white/80 sm:text-base">{heroDescription}</p>
          </div>
        ...
        <div className="relative z-10 mt-9 flex flex-col gap-4 border-t border-white/10 pt-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3 text-sm text-white/70"><CalendarDays className="h-4 w-4 text-[#dfe4ff]" /><span>Looking at</span><MonthYearPicker value={selectedMonth} onChange={setSelectedMonth} /></div>
          <div className="flex items-center gap-2 text-xs text-white/65"><Waves className="h-4 w-4" /> A live view of this workspace</div>
        </div>
      </section>
      ...
      <section className="dashboard-reveal dashboard-reveal-3 grid gap-5 lg:grid-cols-2">
        <div className="premium-card p-6 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Category check-in</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">A quick look at what’s running high.</h2><p className="mt-1 text-sm text-slate-500">Your spending against the limits you’re using this month.</p></div><Bell className="h-5 w-5 text-[#bf7864]" /></div><div className="mt-5 space-y-3">{overBudgetCategories.length ? overBudgetCategories.slice(0, 3).map((category) => <div key={category.rawName} className="flex items-center justify-between gap-3 rounded-2xl border border-[#f0d2c9] bg-[#fff7f4] px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-bold text-[#7c4239]">{category.name} spending is {category.overagePercent}% over budget</p><p className="mt-1 text-xs text-[#a65c4e]">{currency(category.value)} spent vs {currency(category.budget)} budget</p></div><span className="shrink-0 rounded-full bg-[#bf7864] px-2.5 py-1 text-[10px] font-bold text-white">Review</span></div>) : <div className="rounded-2xl border border-dashed border-[#dfe4ff] bg-[#eef0fb] px-4 py-4 text-sm font-semibold text-[#5867bb]">All tracked categories are within their starter budgets.</div>}</div></div>
      ...
      <section className="dashboard-reveal dashboard-reveal-3 grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.9fr)]">
        <div className="premium-card min-h-[410px] p-6 sm:p-7">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div><p className="eyebrow">Cash flow</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Income in, money out.</h2><p className="mt-1 text-sm text-slate-500">A simple view of everything coming in and going out.</p></div>
        ...
        <div className="premium-card p-6 sm:p-7">
          <div className="flex items-start justify-between"><div><p className="eyebrow">Spending pulse</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">Where your money went.</h2></div><div className="rounded-xl bg-[#e9eefb] p-2.5 text-[#5867bb]"><BarChart3 className="h-5 w-5" /></div></div>
      ...
      <section className="dashboard-reveal dashboard-reveal-5 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        ...
        <div className="relative overflow-hidden rounded-[1.5rem] bg-[#222d4b] p-6 text-white shadow-[0_18px_45px_rgba(16,43,41,0.18)] sm:p-7"><div className="absolute -right-14 -top-14 h-40 w-40 rounded-full border-[22px] border-[#aeb9ee]/10" /><div className="absolute -bottom-20 -left-10 h-44 w-44 rounded-full border-[28px] border-orange-200/10" /><div className="relative z-10"><div className="mb-10 flex items-center justify-between"><div className="rounded-xl bg-white/10 p-2.5"><Sparkles className="h-5 w-5 text-[#dfe4ff]" /></div><span className="rounded-full border border-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#dfe4ff]/70">A useful observation</span></div><p className="text-sm font-medium text-[#dfe4ff]/65">{isStreakComplete ? 'Consistency unlocked' : leadingCategory ? 'A clearer money signal' : 'One small shift, big impact'}</p><h3 className="mt-3 text-2xl font-semibold leading-tight tracking-tight">{isStreakComplete ? 'Your spending pattern is ready for a smarter review.' : totals.income ? `Your savings rate is ${savingsRate}% this month.` : leadingCategory ? `${leadingCategory.name} is your biggest spending lever.` : 'Your next best move is consistency, not restriction.'}</h3><p className="mt-4 text-sm leading-6 text-white/70">{isStreakComplete ? 'Amazing consistency! Your spending patterns are now clear. Next step: review your top category this Sunday.' : totals.income && savingsRate >= 0 ? `Great job! You kept ${currency(totals.balance)} after expenses. Review ${leadingCategory?.name || 'your top category'} this Sunday to keep the momentum.` : totals.income ? `Your expenses are ${Math.abs(savingsRate)}% above income. ${leadingCategory ? `Start with ${leadingCategory.name} at ${currency(leadingCategory.value)} and set one calmer limit.` : 'Review your largest category and choose one practical limit.'}` : leadingCategory ? `You spent ${currency(leadingCategory.value)} on ${leadingCategory.name}. Would a ${currency(Math.max(0, Math.round(leadingCategory.value * 0.94)))} target next month create breathing room?` : 'Add one income or expense to unlock a clearer spending pattern.'}</p><button onClick={() => onNavigate?.('insights')} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-bold text-[#222d4b] transition hover:-translate-y-0.5">See the full read <ChevronRight className="h-4 w-4" /></button></div></div>
      </section>

      <section className="dashboard-reveal dashboard-reveal-6"><div className="mb-4 flex items-end justify-between"><div><p className="eyebrow">Useful next steps</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">The things you’ll come back to.</h2></div><p className="hidden text-sm text-slate-500 sm:block">A small set of tools for staying on top of your money.</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Shortcut icon={Target} title="Budget planner" description="Set a calmer monthly plan" onClick={() => onOpenBudgetPlanner ? onOpenBudgetPlanner() : onNavigate?.('budget-planner')} tone="teal" /><Shortcut icon={Trophy} title="Savings goals" description="Turn intent into milestones" onClick={() => onNavigate?.('savings-goals')} tone="violet" /><Shortcut icon={ScanLine} title="Scan a bill" description="Capture the details instantly" onClick={() => onNavigate?.('bill-scanner')} tone="orange" /><Shortcut icon={Lightbulb} title="Money coach" description="Talk through your next step" onClick={() => onNavigate?.('coach')} tone="blue" /></div></section>
    </div>
  );
});
