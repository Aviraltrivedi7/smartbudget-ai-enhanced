...
const MonthlyReport: React.FC<MonthlyReportProps> = ({ onBack, transactions, onImport }) => {
  ...
  return <div className="mx-auto max-w-7xl space-y-7 px-4 py-6 sm:px-6 lg:px-10 lg:py-9"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><button onClick={onBack} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-slate-500 transition hover:text-teal-700"><ArrowLeft className="h-4 w-4" /> Back to overview</button><p className="eyebrow">Reports & exports</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em] text-slate-950 sm:text-5xl">Your money, in a file.</h1><p className="mt-3 max-w-xl text-sm leading-6 text-slate-500">Download a clean report for your records, export raw transactions, or bring in a CSV from another tracker.</p></div><div className="inline-flex items-center gap-2 self-start rounded-xl border border-teal-100 bg-teal-50 px-3.5 py-2.5 text-xs font-bold text-teal-800"><ShieldCheck className="h-4 w-4" /> Private export tools</div></div>
    <section className="grid gap-4 md:grid-cols-3"><ReportStat label="Total income" value={money(report.income)} tone="mint" /><ReportStat label="Total expenses" value={money(report.expenses)} tone="sand" /><ReportStat label="Net savings" value={money(report.savings)} tone="violet" /></section>
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]"><div className="premium-card p-5 sm:p-7"><div className="flex items-start justify-between"><div><p className="eyebrow">Monthly snapshot</p><h2 className="mt-2 text-xl font-semibold text-slate-950">A clear read on your month.</h2></div><div className="rounded-xl bg-teal-50 p-2.5 text-teal-700"><PieChart className="h-5 w-5" /></div></div><div className="mt-7 space-y-4">{report.byCategory.length ? report.byCategory.slice(0, 6).map(([category, amount]) => <div key={category}><div className="mb-2 flex items-center justify-between text-sm"><span className="font-semibold text-slate-700">{category}</span><span className="font-bold text-slate-900">{money(amount)}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-teal-600" style={{ width: `${Math.max(4, (amount / Math.max(report.expenses, 1)) * 100)}%` }} /></div></div>) : <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-400">Add or import transactions to build your report.</div>}</div></div><div className="rounded-[1.75rem] bg-[#102b29] p-6 text-white shadow-[0_18px_45px_rgba(16,43,41,0.18)] sm:p-7"><Sparkles className="h-5 w-5 text-teal-200" /><p className="mt-7 text-[10px] font-bold uppercase tracking-[0.2em] text-teal-100/60">Smart readout</p><h2 className="mt-3 text-2xl font-semibold leading-tight tracking-tight">{report.topCategory === '—' ? 'Your story starts with one transaction.' : `${report.topCategory} is your biggest spending lever.`}</h2><p className="mt-4 text-sm leading-6 text-white/55">{report.topCategory === '—' ? 'Import a CSV or add a transaction to unlock category-level insights.' : `${money(report.expenses)} spent across ${report.byCategory.length} categories, with a ${report.savingsRate.toFixed(1)}% savings rate.`}</p><div className="mt-8 flex items-center gap-2 text-xs font-bold text-teal-100"><CheckCircle2 className="h-4 w-4" /> {transactions.length} transactions included</div></div></section>
    <section className="grid gap-4 md:grid-cols-3"><ExportCard icon={FileText} title="PDF report" description="A polished monthly summary with category insights." action={downloadPdf} label={isGenerating ? 'Generating…' : 'Download PDF'} disabled={isGenerating} tone="dark" /><ExportCard icon={Download} title="CSV export" description="Raw transaction data for analysis and backups." action={downloadCsv} label="Export CSV" tone="mint" /><ExportCard icon={FileUp} title="Import CSV" description="Bring transactions from another spreadsheet or app." action={() => inputRef.current?.click()} label={isImporting ? 'Importing…' : 'Choose CSV file'} disabled={isImporting} tone="orange" /></section><input ref={inputRef} type="file" accept=".csv,text/csv" onChange={handleImport} className="hidden" />
  </div>;
};

const ReportStat = ({ label, value, tone }: { label: string; value: string; tone: string }) => <div className={`metric-card ${tone === 'mint' ? 'metric-card-mint' : tone === 'sand' ? 'metric-card-sand' : 'bg-violet-100 text-violet-950'}`}><p className="text-[10px] font-bold uppercase tracking-[0.18em] opacity-60">{label}</p><p className="mt-4 text-3xl font-semibold tracking-tight">{value}</p></div>;
const ExportCard = ({ icon: Icon, title, description, action, label, disabled, tone }: { icon: React.ElementType; title: string; description: string; action: () => void; label: string; disabled?: boolean; tone: string }) => <div className="premium-card p-5"><div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${tone === 'dark' ? 'bg-[#102b29] text-teal-200' : tone === 'mint' ? 'bg-teal-50 text-teal-700' : 'bg-orange-50 text-orange-600'}`}><Icon className="h-5 w-5" /></div><h3 className="mt-5 text-lg font-bold text-slate-900">{title}</h3><p className="mt-2 min-h-[42px] text-sm leading-5 text-slate-500">{description}</p><button onClick={action} disabled={disabled} className={`mt-5 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 ${tone === 'dark' ? 'bg-[#102b29] text-white hover:bg-teal-800' : 'border border-slate-200 bg-white text-slate-700 hover:border-teal-300 hover:text-teal-700'}`}><Download className="h-4 w-4" />{label}</button></div>;
